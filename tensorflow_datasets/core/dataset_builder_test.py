...
class DatasetBuilderTest(testing.TestCase):
  ...
  @testing.run_in_graph_and_eager_modes()
  def test_load_with_different_format(self):
    with testing.tmp_dir(self.get_temp_dir()) as tmp_dir:
      _, ds_info_1 = load.load(
          name="dummy_dataset_with_configs",
          with_info=True,
          data_dir=tmp_dir,
          download=True,
          split=splits_lib.Split.TRAIN,
          download_and_prepare_kwargs={"file_format": "tfrecord"})
      self.assertEqual(ds_info_1.file_format.name, "TFRECORD")

      # Since the data is already downloaded and prepared (using tfrecord),
      # `load` will load the data from disk.
      _, ds_info_2 = load.load(
          name="dummy_dataset_with_configs",
          with_info=True,
          data_dir=tmp_dir,
          download=True,
          split=splits_lib.Split.TRAIN,
          download_and_prepare_kwargs={"file_format": "riegeli"})
      self.assertEqual(ds_info_2.file_format.name, "TFRECORD")
