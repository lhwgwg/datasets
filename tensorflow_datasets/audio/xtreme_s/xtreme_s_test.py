...
class FleursTest(tfds.testing.DatasetBuilderTestCase):
  ...
  @classmethod
  def setUpClass(cls):
    # Mock out the file path to read the dummy data instead of the actual
    # source data.
    dummy_path = pathlib.Path(__file__).parent / 'dummy_data' / 'fleurs'
    mock.patch.object(cls.DATASET_CLASS, '_FLEURS_DATA_SUBDIRECTORY',
                      str(dummy_path)).start()
