...
class MockFs(object):
  ...
  def _abspath(self, p) -> pathlib.Path:
    """Normalize the path."""
    p = os.fspath(p)
    if p.startswith('/'):  # TODO(epot): Windows compatibility
      p = p[1:]  # Strip leading `/`
    out_p = self._tmp_dir / p
    assert os.fspath(out_p).startswith(os.fspath(self._tmp_dir))  # Sanity check
    return out_p
...
@contextlib.contextmanager
def mock_gfile(**kwargs: Any) -> Iterator[None]:
...
def mock_tf(symbol_name: str, *args: Any, **kwargs: Any) -> Iterator[None]:
  ...
  with contextlib.ExitStack() as stack:
    ...
    for tf_lib in (tf_lib1, tf_lib2):
      ...
      if kwargs:  # Patch each attributes individually
        assert not args
